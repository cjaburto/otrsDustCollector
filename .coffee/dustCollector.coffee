...
casper.then ->
  table = 7
  textRegex = /respuesta automática|]\n\s\s\s\s\s\s\s\s\s\s\s alerta|no se puede entregar|Undeliverable|AntiPortonazo|UPS|Fuera de la oficina|Ausente|Delayed|Entrega retrasada|Análisis Modificaciones Ley/gi
